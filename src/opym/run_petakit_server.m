...
while true
    ...
    try
        ...
        switch jobType
            ...
            otherwise
                % --- DESKEW / DECONVOLUTION JOB ---
                val_psfPath = safelyGetParam(p, 'psf_path', '');

                if ~isempty(val_psfPath)
                    % --- MODE: Deconvolution + Deskew ---
                    logMsg('         Type: Deskew + Deconvolution');
                    ...
                    XR_decon_data_wrapper( ...
                        ...
                        'psfFullpaths', {val_psfPath}, ...
                        'deconIter', val_iter, ...
                        'xyPixelSize', val_xy, ...
                        'dz', val_dz, ...
                        'skewAngle', val_ang, ...
                        'skewed', true, ...     % Essential for OPM
                        'GPUJob', true, ...     % Assumes GPU availability
                        'save16bit', true, ...
                        'parseCluster', false, ...
                        'parseParfor', true, ...
                        'masterCompute', true, ...
                    ...
                    % --- MODE: Deskew Only ---
                    logMsg('         Type: Deskew/Rotate (Only)');
