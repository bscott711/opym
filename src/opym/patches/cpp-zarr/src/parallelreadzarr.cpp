        ...
        for(int64_t f = w*batchSize; f < (w+1)*batchSize; f++){
            ...
            if(!file.is_open()){
                ...
                if(Zarr.get_cname() == "raw"){
                    // Uncompressed chunk (see zarr.cpp's `compressor: null`
                    // handling) -- on-disk bytes are the pixel data as-is,
                    // no blosc/gzip framing to unwrap. A short last-axis
                    // edge chunk is legitimately smaller than `sB` (the
                    // full-chunk size); never copy more than what's really
                    // on disk.
                    dsize = std::min<std::streamsize>(fileLen, (std::streamsize)sB);
                    memcpy(bufferDest, buffer, dsize);
                }
