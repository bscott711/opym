...
zarr_format(2), subfolders({0,0,0}), shard(false), chunk_shape({1,1,1})
{
    ...
    try{
        ...
        if(zarray.at("compressor").is_null()){
            // Uncompressed (raw) zarr v2 array -- spec-legal
            // (https://zarr-specs.readthedocs.io: "compressor": null means
            // chunks are stored as raw bytes), confirmed against a real
            // `.zarray` written by the acquisition pipeline. Both branches
            // below call `.at()` on `compressor` assuming it's always a
            // non-null object (blosc-shaped, then gzip-shaped as a
            // fallback) -- neither succeeds against JSON null, and the
            // resulting exception used to be swallowed by the outer
            // catch(...) and misreported as "Metadata is incomplete".
            // `cname = "raw"` is a sentinel the read path
            // (parallelreadzarr.cpp) checks for to skip decompression
            // entirely and copy chunk bytes through unchanged.
            cname = "raw";
            id = "";
            clevel = 0;
            blocksize = 0;
