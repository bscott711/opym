...
% 1. Prepare PSFs
persistent H psz sq_p gpu_PSF last_useGPU

% First check if PSF has changed or if we need to initialize GPU array
if isempty(sq_p) || last_useGPU ~= useGPU || sq_p ~= sum(PSF .^ 2, 'all') || (useGPU && ~isempty(gpu_PSF) && ~existsOnGPU(gpu_PSF))
    sq_p = sum(PSF .^ 2, 'all');
    last_useGPU = useGPU;
    
    if useGPU
        try
            gpu_PSF = gpuArray(PSF);
        catch
            warning('Failed to push PSF to GPU. Falling back to CPU.');
            useGPU = false;
            last_useGPU = false;
            gpu_PSF = PSF;
        end
    ...
    % Force recompute of H if PSF changed
    psz = []; 
end

% Now check if H needs to be recomputed (e.g. block size changed)
if isempty(psz) || numel(psz) ~= numel(sizeI) || any(psz ~= sizeI) || (useGPU && ~isempty(H) && ~existsOnGPU(H))
    psz = sizeI;
    
    if useGPU
        try
            H = decon_psf2otf(gpu_PSF ./ sum(gpu_PSF(:)), double(sizeI));
        catch
            warning('Failed to compute OTF on GPU. Falling back to CPU.');
            useGPU = false;
            last_useGPU = false;
            gpu_PSF = gather(gpu_PSF);
            H = decon_psf2otf(gpu_PSF ./ sum(gpu_PSF(:)), double(sizeI));
        end
    else
        H = decon_psf2otf(gpu_PSF ./ sum(gpu_PSF(:)), double(sizeI));
