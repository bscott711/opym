...
%   the crop on the MATLAB Parallel Pool (48+ cores).
    ...
    fprintf('🚀 Starting Parallel Crop: %s\n', baseName);
    ...
    try
        ...
        % Warning already suppressed above
    end
    ...
    if isempty(pool)
        n_workers = 1;
        fprintf('⚠️ No parallel pool found. Running serially.\n');
    else
        n_workers = pool.NumWorkers;
    end

    fprintf('📊 Metadata: %d Directories, %d Channels assumed.\n', num_total_dirs, num_channels);
    fprintf('🔥 Launching parallel processing on %d workers...\n', n_workers);
    ...
    parfor k = 1:num_total_dirs
        % Ensure warnings are off on workers too
        warning('off', 'MATLAB:imagesci:Tiff:libraryWarning');
        ...
        % --- Channel Logic (Matches Jupyter Notebook) ---
        % Channels 0, 3 -> Bottom Camera
        % Channels 1, 2 -> Top Camera
    ...
    fprintf('✅ Parallel Crop Complete. Output in: %s\n', outputDir);
end
