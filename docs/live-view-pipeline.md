...
Planes leave the PC while the stack is still being acquired:
...
- When the last plane is exposed, only the final slab (about 6 MB on the
  wire) is still in flight. That is why a full 161-plane volume is on Argus
  0.09 s (p50) / 0.22 s (p95) after its last plane.

Deconvolution and deskew/rotate need the complete volume, so the GPU work
starts when the volume's last slab lands, not before. Both channels come
from the same camera frame (spectral split), so they complete together and
go to the two GPUs in parallel, one channel each.

## The path

```
Acquisition PC (Windows, pymmcore-gui)
  camera frame ── spectral split ──> channel planes (490 × 1458 uint16)
...
Argus (micro001)                                          v
  opym-receive ──> raw OME-Zarr on /dev/shm/opym_stream_stage (1 chunk per plane)
     │   volume complete: ACK to the PC, QC projections, live ticket per (t, c)
     v
  queue_live/ (claimed before the backfill queue; LIVE_LEASE.json holds the backfill off)
     │
  opym-serve: 2 MATLAB PetaKit servers, one per RTX PRO 6000, one channel each
...
Shape: 161 planes × 490 × 1458, 2 channels, dz 0.5 µm, one timepoint every
5–10 s. About 1.6 s to expose the stack.
...
| Staged, ticket queued | ~0.02–0.05 s | The receiver writes the slab, marks (t, c) complete, and the lane writes a `live_zarr` ticket in the same loop pass. |
...
| Layers built | ~0.7 s | Every layer is rebuilt each timepoint; the layer thumbnail alone costs 0.17 s per channel. |
| Painted | ~0.6 s | Two 1 GB 3D textures uploaded to the GPU. |
...
| Choice | Why | Rejected |
|---|---|---|
| **Stream slabs during the stack** | Only the last slab is left to send after the last plane. | Sending each volume after its last plane: both channels (about 180 MB compressed) still in flight, about 0.7 s on 4 links; 13 s uncompressed on the old single tunnel. |
...
| **One ticket per (t, c)** | The two GPUs split a timepoint's channels. | One ticket per timepoint (one GPU per timepoint). |
